...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 use_learnable_pos_emb=False, init_scale=0.001,
                 use_mean_pooling=True, use_rel_pos_bias=False, lin_probe=False, linear_type='standard'):
        super().__init__()
    ...
    def forward_features(self, x, is_train=True):
        ...
        if self.fc_norm is not None:
            if self.lin_probe:
                x = self.fc_norm(x[:, 1:].mean(1), is_train)
            else:
...
def panderm_base_patch16_224_finetune(pretrained, num_classes, drop_rate, drop_path_rate, attn_drop_rate, 
                                    drop_block_rate, use_mean_pooling, init_scale, use_rel_pos_bias, 
                                    init_values, lin_probe, patch_size, **kwargs):
